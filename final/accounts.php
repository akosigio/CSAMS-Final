...
class accounts extends DatabaseAccount {
    ...
    public function getAllMyStudentBySubjectandTeacher($uid, $sid){
        $sql = "";
        $sql = "Select *,`classes`.status from `classes` inner join `users` on `users`.id = `classes`.`stud_id` WHERE date = CURRENT_DATE() AND teacher_id = '$uid' and subject_id = '$sid'";
        if($result = $this->conn->query($sql)){
            // expecting one or more rows
            return $result;
            exit;
    ...
    public function getStudendBySubject($sid){
        $sql = "";
        $sql = "SELECT * FROM `studentsubject` inner join `users` on `users`.id = `studentsubject`.`studid` WHERE `studentsubject`.`subjectid` = '$sid'  order by `users`.`name` DESC";
        if($result = $this->conn->query($sql)){
            // expecting one or more rows
            return $result;
            exit;
